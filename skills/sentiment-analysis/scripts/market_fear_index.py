...
def fetch_vix() -> dict:
    ...
    通过 Yahoo Finance 获取 ^VIX 数据（akshare 的 index_vix 接口已废弃）。
...
def fetch_ashare_sentiment() -> dict:
    ...
    try:
        ...
        # 北向资金（使用新的接口 stock_hsgt_fund_flow_summary_em）
        try:
            ...
            if df is not None and not df.empty:
                ...
                if not north_df.empty:
                    ...
                    today_north = north_df[north_df["交易日"] == latest_date]

                    net_buy = today_north["成交净买额"].sum()
                    # 成交净买额单位可能是元，转换为亿元
                    if abs(net_buy) > 100000:
                        net_buy = net_buy / 1e8  # 元转亿元
                    ...
                    if net_buy is not None:
                        result["north_flow"] = {
                            ...
                            "signal": "外资流入" if net_buy > 0 else "外资流出",
                        }
                        print(f"[北向资金] 净流入: {net_buy:.2f} 亿元")
        except Exception as e:
            print(f"[北向资金] 获取失败: {e}")
