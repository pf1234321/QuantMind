...
def main():
    ...
    parser.add_argument("--db", action="store_true", help="同时写入 MySQL 数据库")
    parser.add_argument("--stock_code", default=None, help="股票代码（写入数据库时使用）")
    ...
    if args.db:
        try:
            ...
            stock_code = args.stock_code or "000000"
            db.save_detail(analyses, stock_code=stock_code, news_date=date.today())
            db.save_aggregate(aggregate, stock_code=stock_code)

            db.close()
            result["db_saved"] = True
            print("[DB] 数据已写入 MySQL")
        except Exception as e:
