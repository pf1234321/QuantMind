...
# 修复 akshare 在新版 pandas（pyarrow 字符串后端）下的兼容性问题
# stock_news_em 内部使用了 r"\u3000" 正则，pyarrow 不支持该转义写法
...
def fetch_stock_notices(stock_code: str, days: int = 7) -> pd.DataFrame:
    """
    获取上市公司公告（通过东方财富公告大全，按股票代码过滤）。
    ...
    try:
        # stock_notice_report 的 symbol 参数是公告类型，不是股票代码
        # 获取全部类型的公告，然后按股票代码筛选
        all_notices = []
        for i in range(days):
            ...
            try:
                ...
                if df is not None and not df.empty and "代码" in df.columns:
                    # 按股票代码过滤
                    df = df[df["代码"].astype(str).str.strip() == str(stock_code)]
                    if not df.empty:
        ...
        if all_notices:
            ...
            print(f"[获取] 公司公告: {stock_code}，共 {len(result)} 条")
            return result
        else:
            print(f"[提示] 未获取到 {stock_code} 的公告")
            return pd.DataFrame()
    except Exception as e:
        print(f"[提示] 获取公司公告接口异常: {e}")
        return pd.DataFrame()
...
def main():
    ...
    if args.stock:
        ...
        # 获取公司公告
        notices = fetch_stock_notices(args.stock, days=args.days)
